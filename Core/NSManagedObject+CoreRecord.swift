    ...
    fileprivate static func transformProperties(_ properties: [String: Any], context: NSManagedObjectContext) -> [String: Any]{
        ...
        for (key, value) in properties {
            ...
            if attrs[localKey] != nil {
                ...
                if CoreRecord.generateRelationships {
                    if rel.isToMany {
...
extension ApplicationRecord {
    ...
    fileprivate static func generateSet(_ rel: NSRelationshipDescription, array: [[String: Any]], context: NSManagedObjectContext) -> NSSet {
        ...
        if CoreRecord.nameToEntities.count > 0 {
            cls = CoreRecord.nameToEntities[rel.destinationEntity!.managedObjectClassName] as? ApplicationRecord.Type
        }
        if cls == nil {
    ...
    fileprivate static func generateObject(_ rel: NSRelationshipDescription, dict: [String: Any], context: NSManagedObjectContext) -> Self {
        ...
        return cls.findOrCreate(dict, context: context) as! Self
    }
    ...
    private static func first(of result: [ApplicationRecord]) -> Self? {
        result.first as? Self
    }
}
